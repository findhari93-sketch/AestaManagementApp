...
import {
  Box, Typography, Card, CardContent, Grid, Alert, FormControl, InputLabel,
  Select, MenuItem, TextField, Button, Paper, Divider, Chip, Table, TableBody,
  TableCell, TableContainer, TableHead, TableRow,
} from '@mui/material'
import {
  TrendingUp, People, AccountBalanceWallet, Business, Download,
...
export default function CompanyReportsPage() {
  ...
  const fetchReportData = async () => {
    try {
      ...
      // Process data
      const totalLaborCost = attendance?.reduce((sum, a) => sum + (a.daily_earnings || 0), 0) || 0
      const totalWorkDays = attendance?.reduce((sum, a) => sum + (a.work_days || 0), 0) || 0
      const uniqueLaborers = new Set(attendance?.map(a => a.laborer_id))
      const avgDailyRate = totalWorkDays > 0 ? totalLaborCost / totalWorkDays : 0

      // Site-wise breakdown
      const siteMap = new Map<string, { cost: number; laborers: Set<string> }>()
      ...
      const siteWiseData = Array.from(siteMap.entries()).map(([name, data]) => ({
      ...
      attendance?.forEach(a => {
        ...
        categoryMap.set(category, (categoryMap.get(category) || 0) + (a.daily_earnings || 0))
      })
      const categoryWiseData = Array.from(categoryMap.entries()).map(([name, value]) => ({ name, value }))

      // Daily trend
      const dailyMap = new Map<string, { cost: number; laborers: Set<string> }>()
      attendance?.forEach(a => {
      ...
      // Top laborers
      const laborerMap = new Map<string, { name: string; days: number; earnings: number }>()
      attendance?.forEach(a => {
        const laborerName = (a.laborers as any)?.name || 'Unknown'
        if (!laborerMap.has(a.laborer_id)) {
          laborerMap.set(a.laborer_id, { name: laborerName, days: 0, earnings: 0 })
        }
            ...
            <Card sx={{ borderRadius: 3 }}>
              <CardContent>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  <Box sx={{ bgcolor: stat.bgColor, color: stat.color, p: 1.5, borderRadius: 2 }}>
                    {stat.icon}
                  </Box>
                </Box>
              </CardContent>
        ...
        <Grid container spacing={3}>
          ...
          <Grid size={{ xs: 12, md: 8 }}>
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              <Typography variant="h6" fontWeight={600} sx={{ mb: 2 }}>Daily Cost Trend</Typography>
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={reportData.dailyTrend}>
                  ...
                  <Legend />
                  <Line type="monotone" dataKey="cost" name="Cost" stroke="#1976d2" strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            </Paper>
          </Grid>
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              <Typography variant="h6" fontWeight={600} sx={{ mb: 2 }}>Cost by Category</Typography>
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
          ...
          <Grid size={{ xs: 12, md: 7 }}>
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              <Typography variant="h6" fontWeight={600} sx={{ mb: 2 }}>Cost by Site</Typography>
              <ResponsiveContainer width="100%" height={300}>
          ...
          <Grid size={{ xs: 12, md: 5 }}>
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              <Typography variant="h6" fontWeight={600} sx={{ mb: 2 }}>Top Earners</Typography>
              <TableContainer>
                <Table size="small">
                  <TableHead>
                    <TableRow>
                  ...
                  <TableBody>
                    {reportData.topLaborers.map((laborer, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell align="right">{laborer.days.toFixed(1)}</TableCell>
                        <TableCell align="right">₹{laborer.earnings.toLocaleString()}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
