...
interface DeleteSettlementConfirmDialogProps {
  open: boolean;
  onClose: () => void;
...
export default function DeleteSettlementConfirmDialog({
  open,
  onClose,
  settlement,
  onSuccess,
}: DeleteSettlementConfirmDialogProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // 2. Reset attendance records - mark as unpaid
      // Get all labor_payments for this settlement
            ...
            <List dense>
              {settlement.weekAllocations.map((alloc, idx) => (
                <ListItem key={idx} sx={{ py: 0.5 }}>
                  ...
                  <ListItemText
                    primary={alloc.weekLabel}
                    secondary={`${formatCurrency(alloc.allocatedAmount)} will be unmarked as paid`}
                  />
                </ListItem>
              ))}
            </List>
