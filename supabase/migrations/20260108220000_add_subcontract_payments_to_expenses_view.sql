-- Migration: Add subcontract_payments to v_all_expenses view
-- Purpose: Include direct subcontract payments in the unified expenses view
-- This fixes the record count discrepancy between Total Expenses and Subcontracts Summary

-- First, ensure we have a 'Contract Payment' expense category
INSERT INTO public.expense_categories (name, module, description)
SELECT 'Contract Payment', 'labor', 'Direct payments to subcontractors (Mesthri/Specialist)'
WHERE NOT EXISTS (
    SELECT 1 FROM public.expense_categories WHERE name = 'Contract Payment'
);

-- Update v_all_expenses view to include subcontract_payments
...
-- Subcontract direct payments (NEW)
-- Direct payments to subcontractors (Mesthri/Specialist contracts)
SELECT
    "sp"."id",
    "sc"."site_id",
    ...
    -- is_cleared based on payment_channel
    CASE
    ...
    "sc"."title" AS "subcontract_title",
    NULL::"uuid" AS "site_payer_id",
    -- payer_name based on payment_channel
    CASE
...
-- Update view comment
COMMENT ON VIEW "public"."v_all_expenses" IS 'Unified view combining regular expenses, derived salary expenses from settlement_groups (Daily Salary aggregated by date), tea shop settlements, miscellaneous expenses, and subcontract direct payments.';
