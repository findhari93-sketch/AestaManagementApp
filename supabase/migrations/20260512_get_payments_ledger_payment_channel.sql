-- 20260512 — Extend get_payments_ledger to return payment_channel so the UI can
-- show a "via wallet" chip on engineer-wallet-settled rows (site-engineer wallet
-- settle feature). Strictly additive: all existing columns unchanged.
...
AS $$
  ...
  SELECT
    ...
    r.is_paid, r.is_pending, r.laborer_id, r.period, r.payment_channel
...
'Unified ledger feed for /site/payments tabs. paid_dm/paid_wk now split by labor_payments.is_under_contract (matches v_all_expenses + useSettlementsList) so contract settlements with linked contract-laborer attendance show only in the Contract bucket. 2026-05-12: added payment_channel column so UI can render "via wallet" chip on engineer-wallet-settled rows.';
