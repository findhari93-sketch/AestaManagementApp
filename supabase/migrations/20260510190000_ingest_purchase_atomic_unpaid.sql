-- Revert is_paid=true from ingest_purchase_atomic.
--
-- AI-ingested bills should land in Material Settlements as Pending so the user
-- can record payment details (mode, payer, date) via the existing settle UI.
-- Setting is_paid=true without payment info skips that step and produces
-- incomplete settlement records.
--
-- Also backfills the two rows created while is_paid=true was active.

UPDATE material_purchase_expenses
SET is_paid = false, paid_date = NULL
WHERE ref_code IN ('MAT-260510-504E', 'MAT-260510-9A01');
...
BEGIN
  ...
  IF NOT v_catalog_only THEN
    ...
    -- created_by → auth.users(id): use auth.uid()
    -- is_paid stays false (default) — user settles via Material Settlements UI
    -- source='ai_ingest' enables the Origin filter on /site/material-expenses
    INSERT INTO material_purchase_expenses (
      ...
      payment_reference, bill_url, status, notes, created_by, source
    )
    VALUES (
