-- Migration: Add settled material_purchase_expenses to v_all_expenses view
-- Purpose: Include settled material purchases in the unified expenses view
-- This fixes the Material tab showing "No records" in Site Expenses

-- First, ensure we have a 'Material Purchase' expense category
INSERT INTO public.expense_categories (name, module, description)
SELECT 'Material Purchase', 'material', 'Material purchase settlements from POs and direct purchases'
WHERE NOT EXISTS (
    SELECT 1 FROM public.expense_categories WHERE name = 'Material Purchase'
);

-- Drop and recreate v_all_expenses view to include material_purchase_expenses
-- Note: DROP is needed because we can't change column types with CREATE OR REPLACE
...
-- Settled Material Purchases (NEW)
...
-- Update view comment
COMMENT ON VIEW "public"."v_all_expenses" IS 'Unified view combining regular expenses, derived salary expenses from settlement_groups (Daily Salary aggregated by date), tea shop settlements, miscellaneous expenses, subcontract direct payments, and settled material purchases.';
