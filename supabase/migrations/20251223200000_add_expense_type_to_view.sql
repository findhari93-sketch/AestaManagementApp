-- Migration: Add expense_type column to v_all_expenses view
-- Purpose: Show human-readable expense type (Daily Salary, Contract Salary, Advance, Material, etc.)
-- This allows distinguishing between different types of expenses in the daily expenses page

-- Must drop and recreate since we're adding a column in the middle
...
SELECT
  ...
  -- NEW: expense_type based on module
  CASE e.module
    WHEN 'labor' THEN 'Daily Salary'
...
SELECT
  ...
  -- NEW: expense_type - distinguish daily vs contract vs advance
  CASE
...
-- Add comment
COMMENT ON VIEW v_all_expenses IS 'Unified view combining regular expenses with derived salary expenses from settlement_groups. Includes expense_type column to distinguish Daily Salary, Contract Salary, Advance, Material, Machinery, General expenses.';
