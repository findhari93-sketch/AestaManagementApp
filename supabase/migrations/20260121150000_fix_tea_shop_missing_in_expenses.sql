-- Migration: Fix tea shop settlements not appearing when shop doesn't exist
-- Problem: Tea shop settlements with invalid tea_shop_id don't appear in v_all_expenses
-- Solution: Remove the WHERE clause that requires shop to exist, show "Unknown Shop" instead
...
-- FIXED: Include settlements even if shop doesn't exist (show "Unknown Shop")
-- Removed WHERE clause requirement that shop must exist
SELECT "ts"."id",
    ...
    "ts"."amount_paid" AS "amount",
        CASE
            WHEN (("ts"."notes" IS NOT NULL) AND ("ts"."notes" <> ''::"text")) THEN
                ('Tea Shop - '::"text" || COALESCE(("tsa"."shop_name")::"text", ("new_ts"."name")::"text", 'Unknown Shop'::"text") || ' - '::"text" || "ts"."notes")
            ELSE
                ('Tea Shop - '::"text" || COALESCE(("tsa"."shop_name")::"text", ("new_ts"."name")::"text", 'Unknown Shop'::"text"))
        END AS "description",
    ( SELECT "expense_categories"."id"
           FROM "public"."expense_categories"
    ...
    NULL::"uuid" AS "site_payer_id",
        ...
        END AS "payer_name",
    "ts"."payment_mode",
    -- Shop name from either table, or "Unknown Shop" if neither exists
    COALESCE("tsa"."shop_name", "new_ts"."name", 'Unknown Shop') AS "vendor_name",
...
WHERE (COALESCE("ts"."is_cancelled", false) = false)
  -- REMOVED: AND ("tsa"."id" IS NOT NULL OR "new_ts"."id" IS NOT NULL)
  -- Now includes settlements even if shop doesn't exist
...
-- Update view comment
COMMENT ON VIEW "public"."v_all_expenses" IS 'Unified view combining regular expenses, derived salary expenses from settlement_groups (Daily Salary aggregated by date), tea shop settlements (supporting both legacy tea_shop_accounts AND new tea_shops tables, even if shop does not exist), miscellaneous expenses, subcontract direct payments, and settled material purchases.';
