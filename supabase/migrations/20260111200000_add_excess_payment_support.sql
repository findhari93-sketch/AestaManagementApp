-- Migration: Add excess/overpayment support to v_all_expenses view
-- Purpose:
--   1. Add 'excess' to the payment_type check constraint
--   2. Fix existing overpayments with payment_type='salary' but no linked labor_payments
--   3. Add a new section for 'excess' payment type in v_all_expenses

-- Step 1: Update the check constraint to allow 'excess' payment type
...
ALTER TABLE "public"."settlement_groups" ADD CONSTRAINT "settlement_groups_payment_type_check"
  CHECK (payment_type IN ('salary', 'advance', 'other', 'excess'));

-- Step 2: Fix existing overpayments that have payment_type='salary' but no labor_payments
-- These are contract salary overpayments recorded when all dues were already settled
UPDATE "public"."settlement_groups"
SET "payment_type" = 'excess'
...
-- Step 2: Drop and recreate the v_all_expenses view with excess payment support
...
-- Update view comment
COMMENT ON VIEW "public"."v_all_expenses" IS 'Unified view combining regular expenses, individual daily salary settlements, contract salary settlements, advance payments, excess/overpayments, tea shop settlements, miscellaneous expenses, and subcontract direct payments. Each settlement is shown as an individual record.';
