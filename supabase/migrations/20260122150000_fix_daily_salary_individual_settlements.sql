-- Migration: Fix Daily Salary settlements to show individual records
-- Purpose: Change Daily Salary from aggregated-by-date back to individual records
-- This fixes the issue where SET-260113-002 (Rs.500) was showing as Rs.1,000 in expenses
-- because it was aggregating all settlements for the date instead of showing individual settlements

-- The 20260120100000_add_material_expenses_to_view migration accidentally reverted
-- the fix from 20260110100000_show_individual_daily_settlements by re-introducing
-- GROUP BY for Daily Salary settlements
...
-- Daily Salary settlements (FIXED: individual records, NOT aggregated by date)
-- This ensures each settlement reference shows its correct individual amount
SELECT "sg"."id",
    "sg"."site_id",
...
-- Update view comment
COMMENT ON VIEW "public"."v_all_expenses" IS 'Unified view combining regular expenses, INDIVIDUAL daily salary settlements (not aggregated), contract salary settlements, advance payments, tea shop settlements, miscellaneous expenses, subcontract direct payments, and settled material purchases. Each settlement shows its correct individual amount.';