-- Migration: Fix tea_shop_settlements to support both legacy tea_shop_accounts AND new tea_shops tables
-- Problem: Settlements created with company tea shops (tea_shops table) don't appear in v_all_expenses
-- because the view only JOINs with tea_shop_accounts table.
--
-- Solution: Use LEFT JOINs with both tables and COALESCE the values.
-- Also use ts.site_id (settlement's own site_id) as priority for grouped sites.

-- First, let's update the view to handle both tea shop table structures
...
-- FIXED: Support both legacy tea_shop_accounts AND new tea_shops tables
-- Priority for site_id: subcontract > settlement's own site_id > legacy tea_shop_account > new tea_shop via assignments
SELECT "ts"."id",
    -- Site ID priority: subcontract's site > settlement's own site_id > legacy tsa.site_id > assignment site_id
    COALESCE(
        "sc"."site_id",                    -- 1. Subcontract's site_id (if linked)
        "ts"."site_id",                    -- 2. Settlement's own site_id (for group mode)
        "tsa"."site_id",                   -- 3. Legacy tea_shop_account site_id
        "tsa_assign"."site_id"             -- 4. From tea_shop_site_assignments
    ) AS "site_id",
    "ts"."payment_date" AS "date",
    "ts"."payment_date" AS "recorded_date",
    "ts"."amount_paid" AS "amount",
        CASE
            WHEN (("ts"."notes" IS NOT NULL) AND ("ts"."notes" <> ''::"text")) THEN
                ('Tea Shop - '::"text" || COALESCE(("tsa"."shop_name")::"text", ("new_ts"."name")::"text", 'Unknown'::"text") || ' - '::"text" || "ts"."notes")
            ELSE
                ('Tea Shop - '::"text" || COALESCE(("tsa"."shop_name")::"text", ("new_ts"."name")::"text", 'Unknown'::"text"))
        END AS "description",
    ( SELECT "expense_categories"."id"
           FROM "public"."expense_categories"
    ...
    NULL::"uuid" AS "site_payer_id",
        ...
        END AS "payer_name",
    "ts"."payment_mode",
    -- Shop name from either table
    COALESCE("tsa"."shop_name", "new_ts"."name") AS "vendor_name",
...
FROM "public"."tea_shop_settlements" "ts"
    -- LEFT JOIN with legacy tea_shop_accounts
    LEFT JOIN "public"."tea_shop_accounts" "tsa" ON ("ts"."tea_shop_id" = "tsa"."id")
    -- LEFT JOIN with new tea_shops table
    LEFT JOIN "public"."tea_shops" "new_ts" ON ("ts"."tea_shop_id" = "new_ts"."id")
    -- LEFT JOIN to get site assignment for new tea_shops (prioritize direct site assignment)
    LEFT JOIN LATERAL (
        SELECT "tsa2"."site_id", "tsa2"."site_group_id"
        FROM "public"."tea_shop_site_assignments" "tsa2"
        WHERE "tsa2"."tea_shop_id" = "ts"."tea_shop_id"
          AND "tsa2"."is_active" = true
        ORDER BY
            CASE WHEN "tsa2"."site_id" IS NOT NULL THEN 0 ELSE 1 END, -- Prefer direct site over group
            "tsa2"."assigned_at" DESC
        LIMIT 1
    ) "tsa_assign" ON true
    -- LEFT JOIN with subcontracts
    LEFT JOIN "public"."subcontracts" "sc" ON ("ts"."subcontract_id" = "sc"."id")
WHERE (COALESCE("ts"."is_cancelled", false) = false)
  -- Ensure at least one tea shop table matched
  AND ("tsa"."id" IS NOT NULL OR "new_ts"."id" IS NOT NULL)

UNION ALL

-- Miscellaneous expenses
-- FIXED: Use subcontract's site_id when linked to a subcontract
SELECT
    "me"."id",
    COALESCE("sc"."site_id", "me"."site_id") AS "site_id",  -- Use subcontract's site_id if linked
    "me"."date",
...
-- Shows material purchases that have been settled (paid or have settlement_date)
SELECT
    "mpe"."id",
    "mpe"."site_id",
...
-- Update view comment
COMMENT ON VIEW "public"."v_all_expenses" IS 'Unified view combining regular expenses, derived salary expenses from settlement_groups (Daily Salary aggregated by date), tea shop settlements (supporting both legacy tea_shop_accounts AND new tea_shops tables), miscellaneous expenses, subcontract direct payments, and settled material purchases.';
