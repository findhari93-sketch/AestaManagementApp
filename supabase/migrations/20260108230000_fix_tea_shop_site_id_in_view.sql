-- Migration: Fix tea_shop_settlement site_id in v_all_expenses
-- Purpose: When a tea_shop_settlement is linked to a subcontract, use the subcontract's site_id
-- This ensures settlements show up in the correct site's expenses

-- Update v_all_expenses view
...
-- FIXED: Use subcontract's site_id when linked to a subcontract, otherwise use tea_shop_account's site_id
SELECT "ts"."id",
    COALESCE("sc"."site_id", "tsa"."site_id") AS "site_id",  -- Use subcontract's site_id if linked
...
-- Update view comment
COMMENT ON VIEW "public"."v_all_expenses" IS 'Unified view combining regular expenses, derived salary expenses from settlement_groups (Daily Salary aggregated by date), tea shop settlements (using subcontract site_id when linked), miscellaneous expenses, and subcontract direct payments.';
