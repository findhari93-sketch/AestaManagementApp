-- Migration: Fix payer_source NULL display in Daily Expenses
-- Problem: Settlements without payer_source show "-" instead of a default value
-- Solution:
--   1. Backfill NULL payer_source to 'own_money' (most common default)
--   2. Update view to handle NULL as 'Own Money'

-- ============================================================================
-- 1. Backfill existing NULL payer_source values to 'own_money'
-- ============================================================================
UPDATE settlement_groups
SET payer_source = 'own_money'
WHERE payer_source IS NULL
...
BEGIN
  ...
  RAISE NOTICE 'Updated % settlement_groups records with NULL payer_source to own_money', updated_count;
END $$;

-- ============================================================================
-- 2. Update v_all_expenses view to handle NULL payer_source as 'Own Money'
-- ============================================================================
...
SELECT
  ...
  -- Convert payer_source to human-readable label (with NULL handling - default to 'Own Money')
  CASE
...
-- Add comment
COMMENT ON VIEW v_all_expenses IS 'Unified view combining regular expenses with derived salary expenses from settlement_groups. Labor expenses are ONLY from settlement_groups to ensure all have settlement_reference. NULL payer_source defaults to Own Money.';
