-- Fix: AI-ingested expenses must be marked is_paid=true so they appear on
-- /site/material-expenses. useSiteLevelMaterialExpenses filters own-site
-- expenses with .eq("is_paid", true). Bills captured via AI ingest are
-- historical (already received), so they should surface immediately.
...
BEGIN
  ...
  IF NOT v_catalog_only THEN
    ...
    -- is_paid=true: AI ingest captures an already-received bill; mark paid
    -- immediately so it surfaces on /site/material-expenses (which filters
    -- own-site expenses with is_paid=true).
