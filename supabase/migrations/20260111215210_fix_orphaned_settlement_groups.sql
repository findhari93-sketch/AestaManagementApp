-- Migration: Fix v_all_expenses to exclude orphaned settlement groups
-- Purpose: Only include settlement groups that have actual linked attendance/payment records
-- This fixes the count mismatch between Payments and Expenses pages

-- Drop and recreate the v_all_expenses view with orphaned record filtering
...
-- NEW: Also excludes orphaned settlement groups (no linked attendance records)
SELECT "sg"."id",
    "sg"."site_id",
...
WHERE (("sg"."is_cancelled" = false)
    ...
    -- NEW: Only include if has at least one linked attendance record
    AND (
