-- Migration: Fix group_stock purchases showing incorrectly in v_all_expenses
-- Problem: Raw group_stock purchases were showing in All Site Expenses before settlement
-- Solution: Only show group_stock purchases that have been settled (have settlement_reference)
--
-- Business Logic:
-- - own_site purchases: Show when is_paid = true OR settlement_date IS NOT NULL
-- - group_stock purchases: Only show when settlement_reference IS NOT NULL
--   (This means the self-use portion has been attributed or it's an allocated expense)

-- Drop and recreate v_all_expenses view with fixed material purchase filter
...
-- FIXED: Exclude raw group_stock purchases - they should only appear after settlement
-- Group stock purchases appear in expenses ONLY when:
-- 1. settlement_reference IS NOT NULL (means it's been through inter-site settlement)
-- This ensures:
-- - Own site purchases show when is_paid = true
-- - Group stock self-use portions show when settlement_reference = 'SELF-USE'
-- - Allocated expenses from group settlement show when settlement_reference has settlement code
SELECT
    "mpe"."id",
    "mpe"."site_id",
    COALESCE("mpe"."settlement_date", "mpe"."purchase_date") AS "date",
    "mpe"."purchase_date" AS "recorded_date",
    -- Use the actual amount paid (after bargaining) when available, otherwise use total_amount
...
WHERE (
    -- Include settled material purchases (is_paid = true OR settlement_date is set)
    (("mpe"."is_paid" = true) OR ("mpe"."settlement_date" IS NOT NULL))
    AND (
        -- EXCLUDE raw group_stock purchases that haven't been settled
        -- Only include group_stock if it has a settlement_reference (meaning settled/allocated)
...
-- Update view comment
COMMENT ON VIEW "public"."v_all_expenses" IS 'Unified view combining regular expenses, derived salary expenses from settlement_groups (Daily Salary aggregated by date), tea shop settlements, miscellaneous expenses, subcontract direct payments, and settled material purchases. Material purchases use amount_paid (bargained amount) when available. Group stock purchases are excluded until settled via inter-site settlement.';

-- Add index to improve performance of the new filter
CREATE INDEX IF NOT EXISTS idx_material_purchase_expenses_purchase_type
ON "public"."material_purchase_expenses" ("purchase_type");

CREATE INDEX IF NOT EXISTS idx_material_purchase_expenses_settlement_reference
ON "public"."material_purchase_expenses" ("settlement_reference");
