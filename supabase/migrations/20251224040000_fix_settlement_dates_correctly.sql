-- Migration: Fix settlement dates correctly
-- Purpose:
--   Settlement Date = When money was actually given to the laborer (actual_payment_date)
--   Recorded Date = When the record was entered in the app (created_at)
--
-- The previous migration incorrectly used attendance dates. This fix uses the correct dates.

-- ============================================================================
-- 1. Fix settlement_date to equal actual_payment_date for all settlement_groups
-- ============================================================================
...
-- ============================================================================
-- 2. Update the view to use created_at for recorded_date
-- ============================================================================
...
SELECT
  ...
  e.site_id,
  e.date,
  e.created_at::DATE as recorded_date,  -- When entered in app
  e.amount,
  e.description,
  e.category_id,
...
SELECT
  sg.id,
  sg.site_id,
  -- Settlement Date = When money was given to laborer (actual_payment_date or settlement_date)
  COALESCE(sg.actual_payment_date, sg.settlement_date) as date,
  -- Recorded Date = When entered in the app
  ...
  END as is_cleared,
  CASE
    WHEN sg.payment_channel = 'direct' THEN COALESCE(sg.actual_payment_date, sg.settlement_date)
    WHEN sg.engineer_transaction_id IS NOT NULL THEN
...
-- Add comment
COMMENT ON VIEW v_all_expenses IS 'Unified expenses view. Settlement Date = when money was given. Recorded Date = when entered in app.';
...
-- ============================================================================
COMMENT ON COLUMN settlement_groups.settlement_date IS 'The date when money was given to the laborer. Should equal actual_payment_date.';
COMMENT ON COLUMN settlement_groups.actual_payment_date IS 'The date when payment was made to the laborer.';
