-- Engineer Wallet v2 — Migration F: Recreate v_all_expenses with v2-compatible expressions
--
-- v1 of this view computed is_cleared / cleared_date for engineer-wallet payments by reaching
-- into site_engineer_transactions.is_settled and .confirmed_at — columns that don't exist in v2
-- (they encoded the per-batch settlement workflow). In v2 the wallet ledger has no pending state:
-- the moment a `spend` row exists, money has moved. So:
--   - is_cleared:  engineer_transaction_id IS NOT NULL → true (always cleared)
--   - cleared_date: SELECT transaction_date from the linked spend row
--
-- Migration B already dropped this view; this migration recreates it.
...
COMMENT ON VIEW v_all_expenses IS
  'Unified expenses view across labor, material, tea shop, misc, and subcontract domains. v2 (2026-05-09): wallet-funded entries are considered cleared the moment the spend ledger row exists; cleared_date reads transaction_date from the linked site_engineer_transactions row.';
