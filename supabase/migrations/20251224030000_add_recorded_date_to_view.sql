-- Migration: Add recorded_date column to v_all_expenses view
-- Purpose: Show both settlement_date (work date) and recorded_date (when payment was processed)
-- This gives users full visibility into both dates and avoids confusion
...
SELECT
  sg.id,
  sg.site_id,
  sg.settlement_date as date,  -- Work/attendance date
...
-- Add comment
COMMENT ON VIEW v_all_expenses IS 'Unified view combining regular expenses with derived salary expenses. Includes both date (settlement/work date) and recorded_date (when payment was processed).';
